...
/**
 * @fileOverview Institutional Portrait PDF Layout Hub v21.0.
 * FIXED: Resolved bottom clipping by compacting vertical nodes and reducing rank scale.
...
export default function ReportPDF(props: ReportPDFProps) {
  ...
  return (
    ...
    >
      {/* 1. Header Hub */}
      <div className="w-full flex items-center justify-between px-10 py-5 border-b border-slate-100 bg-slate-50/20 shrink-0">
      ...
      <div className="w-full flex-1 p-8 space-y-6 flex flex-col items-center overflow-hidden">
         ...
         <div className="w-full bg-white rounded-[2rem] p-8 border border-slate-100 flex flex-col items-center justify-center shadow-sm relative overflow-hidden text-center min-h-[160px]">
            ...
            <div className="pt-6 border-t border-slate-100 mt-6 w-full max-w-xl mx-auto grid grid-cols-3 gap-6">
               ...
               <div className="flex flex-col items-center gap-1 border-x border-slate-50 px-2">
               ...
               <div className="flex flex-col items-center gap-1">
                  <p className="text-[8px] font-bold text-slate-300 uppercase tracking-widest">Attempt #</p>
         ...
         <div className="w-full bg-[#0F172A] rounded-[2rem] p-10 text-white text-center flex flex-col items-center justify-center shadow-2xl relative overflow-hidden min-h-[280px]">
            ...
            <div className="relative z-10 space-y-6 flex flex-col items-center w-full">
               <div className="space-y-2">
                  <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em]">Your Punjab Rank</p>
                  <span className="text-[100px] font-[900] tabular-nums tracking-tighter leading-none block drop-shadow-[0_20px_40px_rgba(37,99,235,0.4)]">
                     #{rank}
                  </span>
               </div>
               
               <div className="flex flex-col items-center justify-center gap-3">
                  <span className="text-[18px] font-bold text-slate-500 tabular-nums uppercase tracking-[0.1em]">
                     / {totalCandidates.toLocaleString()} Candidates
                  </span>
                  <Badge className="bg-emerald-500 text-white border-none font-black text-[9px] px-8 py-2.5 rounded-full shadow-2xl uppercase tracking-widest">
         ...
         {subjects.length > 0 && (
            <div className="w-full space-y-3">
               <p className="text-[9px] font-black text-slate-400 text-center uppercase tracking-widest">Subject Analysis</p>
                     ...
                     <tbody className="divide-y divide-slate-50">
                        {subjects.slice(0, 3).map((s, i) => (
                           <tr key={i} className="h-10 hover:bg-slate-50/30 transition-colors">
                              <td className="px-8 font-bold text-[12px] text-[#0F172A] text-left">{s.name}</td>
                              <td className="px-4 text-center font-black text-primary text-sm tabular-nums">{Number(s.score).toFixed(1)}</td>
                              <td className="px-4 text-center">
                                 <Badge className="bg-emerald-50 text-emerald-600 border-none font-black text-[9px] tabular-nums px-3 py-0.5 rounded-lg shadow-sm">{s.accuracy}%</Badge>
                              </td>
                           </tr>
                        ))}
                     </tbody>
         ...
         {/* 7. Footer Node - MOVED UP */}
         <div className="pt-6 border-t border-slate-100 flex items-center justify-between mt-auto w-full pb-2">
            <div className="flex items-center gap-5">
               ...
               <div className="space-y-1 text-left">
                  <p className="text-primary font-black text-xs flex items-center gap-2 leading-none uppercase">
                     <ShieldCheck className="h-3.5 w-3.5" /> Digitally Verified
                  </p>
                  <p className="text-slate-400 text-[9px] max-w-[300px] leading-relaxed font-medium">
...
function CountPill({ label, val, color }: any) {
   return (
      <div className={cn("p-4 rounded-2xl flex flex-col items-center justify-center gap-1 h-16", color)}>
