...
/**
 * @fileOverview Institutional Portrait PDF Layout Hub v22.0.
 * FIXED: Absolute vertical centering for all main nodes.
 * FIXED: Overlap resolution in Rank node with explicit spacers.
...
export default function ReportPDF(props: ReportPDFProps) {
  ...
  return (
    ...
    >
      {/* 1. HEADER HUB */}
      <div className="w-full flex items-center justify-between px-12 py-6 border-b border-slate-100 bg-slate-50/20 shrink-0">
      ...
      <div className="w-full flex-1 p-10 space-y-8 flex flex-col items-center overflow-hidden">
         
         {/* 2. CANDIDATE IDENTITY BOX - CENTERED */}
         ...
         {/* 3. MERIT STANDING - CENTERED WITH OVERLAP RESOLUTION */}
         <div className="w-full bg-[#0F172A] rounded-[2.5rem] p-12 text-white text-center flex flex-col items-center justify-center shadow-2xl relative overflow-hidden min-h-[320px]">
            ...
            <div className="relative z-10 flex flex-col items-center w-full">
               ...
               <div className="flex flex-col items-center gap-2">
                  <span className="text-[120px] font-[900] tabular-nums tracking-tighter leading-none block drop-shadow-[0_20px_40px_rgba(37,99,235,0.4)]">
                     #{rank}
                  </span>
                  ...
                  <span className="text-[22px] font-black text-slate-500 tabular-nums uppercase tracking-[0.2em]">
                  ...
                  <table className="w-full text-center">
                     <thead>
                        <tr className="bg-slate-50 border-b border-slate-100 h-12">
                           <th className="px-12 font-black text-[11px] text-slate-500 uppercase tracking-widest text-left">Subject hub</th>
                           <th className="px-4 font-black text-[11px] text-center text-slate-500 uppercase tracking-widest">Net score</th>
                           <th className="px-12 font-black text-[11px] text-right text-slate-500 uppercase tracking-widest">Accuracy</th>
                        </tr>
                     </thead>
                     <tbody className="divide-y divide-slate-50">
                        {subjects.slice(0, 4).map((s, i) => (
                           <tr key={i} className="h-12 hover:bg-slate-50/30 transition-colors">
                              <td className="px-12 font-bold text-[13px] text-[#0F172A] text-left">{s.name}</td>
                              ...
                              <td className="px-12 text-right">
                                 <Badge className="bg-emerald-50 text-emerald-600 border-none font-black text-[10px] tabular-nums px-4 py-1 rounded-lg shadow-sm">{s.accuracy}%</Badge>
                              </td>
                           </tr>
                        ))}
                     </tbody>
         ...
         {/* 7. FOOTER HUB - POSITIONED FOR VISIBILITY */}
         <div className="pt-8 border-t border-slate-100 flex items-center justify-between mt-auto w-full pb-4">
            <div className="flex items-center gap-6">
               <div className="h-20 w-20 bg-white border-2 border-slate-100 p-2 rounded-2xl shadow-xl relative overflow-hidden">
                  <img src={qrUrl} alt="Verify" className="h-full w-full object-contain" crossOrigin="anonymous" />
               </div>
               <div className="space-y-1.5 text-left">
                  <p className="text-primary font-black text-[13px] flex items-center gap-2 leading-none uppercase">
                     <ShieldCheck className="h-4 w-4" /> Digitally Verified Node
                  </p>
                  <p className="text-slate-400 text-[10px] max-w-[340px] leading-relaxed font-medium">
                     This report node is synchronized with the Cracklix institutional registry. Scan the QR code to verify authenticity on cracklix.in.
                  </p>
               </div>
            </div>
...
function KPIBox({ label, val, color }: any) {
   return (
      <div className="bg-white border border-slate-100 p-5 rounded-2xl flex flex-col items-center justify-center gap-1.5 shadow-sm text-center h-28 flex-1">
...
function CountPill({ label, val, color }: any) {
   return (
      <div className={cn("p-5 rounded-2xl flex flex-col items-center justify-center gap-1 h-18 flex-1", color)}>
