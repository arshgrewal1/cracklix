...
/**
 * @fileOverview Official Institutional Result Report v1.0
 * REBUILT FROM SCRATCH as a professional examination authority scorecard.
 * Optimized for: A4 PDF, WhatsApp Share, and Instagram Story Crop.
 */
const ShareableResultCard = forwardRef<HTMLDivElement, ShareableResultCardProps>(({ data, rank, totalCandidates }, ref) => {
  if (!data) return null;

  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent('https://cracklix.in/results/view?id=' + data.mockId + '&attemptId=' + data.attemptId)}`;
    ...
    >
      ...
      <div className="flex justify-between items-start mb-10 border-b-2 border-slate-100 pb-10">
         <div className="space-y-6">
            ...
            <div className="space-y-1">
               ...
               <p className="text-sm font-bold text-slate-400 uppercase tracking-[0.2em]">Candidate ID: {data.userId?.slice(-12).toUpperCase()}</p>
            </div>
         </div>
         ...
         <div className="text-right space-y-4">
            <div className="inline-flex items-center gap-2 px-6 py-2.5 bg-[#E6F9F3] text-[#10B981] rounded-full border border-[#DCFCE7] shadow-sm">
               ...
               <span className="font-bold text-sm uppercase tracking-widest">Verified Result</span>
            </div>
            <div className="space-y-1 pt-2">
               <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Attempt Date</p>
      ...
      <div className="grid grid-cols-12 gap-8 mb-10">
         <div className="col-span-8 space-y-2">
            <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em]">Test Series Hub</p>
            <h1 className="text-[28px] font-[700] text-[#0F172A] leading-tight uppercase tracking-tight">
               {data.mockTitle}
            </h1>
         </div>
      ...
      {/* 3. MAIN SCORE HUB - PREMIUM TESTBOOK STYLE */}
      <Card className="border-none shadow-xl rounded-[24px] bg-[#F0F7FF] p-10 mb-10 flex justify-between items-center relative overflow-hidden">
         ...
         <div className="flex-1 border-r border-blue-200/50 space-y-6">
            <p className="text-xs font-black text-blue-400 uppercase tracking-[0.4em]">Sectional Score</p>
            ...
            <div className="flex items-center gap-3">
               ...
               <span className="text-sm font-bold text-blue-400 uppercase tracking-widest">Aggregate Percentage</span>
            </div>
         </div>

         <div className="px-16 space-y-8 text-right">
            <div className="space-y-1">
               <p className="text-xs font-black text-blue-400 uppercase tracking-[0.4em]">Punjab Rank</p>
            ...
            <div className="space-y-1">
               <p className="text-xs font-black text-blue-400 uppercase tracking-[0.4em]">Percentile</p>
      ...
      <div className="space-y-6 mb-10">
         <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em] ml-1">Subject Performance</h3>
         <Card className="border border-[#E5EAF2] shadow-sm rounded-[20px] bg-white overflow-hidden">
            <table className="w-full text-left border-collapse">
               <thead className="bg-slate-50">
                  <tr className="h-12">
                     <th className="px-8 font-bold text-[10px] text-slate-400 uppercase tracking-widest">Subject Hub</th>
                     <th className="px-4 font-bold text-[10px] text-slate-400 text-center uppercase tracking-widest">Correct</th>
                     <th className="px-4 font-bold text-[10px] text-slate-400 text-center uppercase tracking-widest">Wrong</th>
                     <th className="px-8 font-bold text-[10px] text-slate-400 text-right uppercase tracking-widest">Net Score</th>
                  </tr>
               </thead>
                     ...
                     <tr key={i} className="h-16">
                        <td className="px-8">
                           <div className="space-y-2">
                              <p className="font-bold text-sm text-[#0F172A] uppercase tracking-tight">{s.name}</p>
      ...
      {/* 6. COMPETITION SNAPSHOT & INSIGHTS */}
      <div className="grid grid-cols-2 gap-8 mb-10">
         <div className="space-y-6">
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em] ml-1">Competition Hub</h3>
            <Card className="border border-[#E5EAF2] p-8 rounded-[20px] grid grid-cols-2 gap-6 bg-white shadow-sm">
               ...
               <div className="col-span-2 pt-4 border-t border-slate-50 flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase text-slate-400">Readiness Hub</span>
                  <Badge className={cn("border-none px-4 py-1 rounded-lg font-black text-[10px] uppercase", readinessStatus.bg, readinessStatus.color)}>
         ...
         <div className="space-y-6">
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em] ml-1">Smart Insights</h3>
      ...
      <div className="mt-auto pt-10 border-t-2 border-slate-100 flex justify-between items-end">
         <div className="space-y-6 text-left">
            <div className="flex items-center gap-4 text-slate-400">
               ...
               <div className="space-y-1">
                  <p className="text-lg font-bold uppercase tracking-tight text-[#0F172A]">Official Report Hub</p>
                  <p className="text-xs font-medium text-slate-400">Digitally verified by Arsh Grewal Management Registry</p>
               </div>
            </div>
            <div className="space-y-1 pt-2">
               <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.4em]">Report ID</p>
               <code className="text-xs font-mono font-bold text-primary">{data.attemptId || "NODE_01_VERIFIED"}</code>
            </div>
         </div>
         <div className="flex items-center gap-10">
            <div className="text-right space-y-1">
               <p className="text-xl font-black text-[#0F172A] uppercase tracking-tight">Verify Registry</p>
               <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Scan to validate node</p>
            </div>
   ...
   return (
      <div className={cn("px-5 py-3 rounded-xl flex items-center gap-3 border border-transparent shadow-sm", bg)}>
         ...
         <div className="min-w-0">
            <span className={cn("text-[8px] font-black uppercase tracking-widest block", color)}>{label}</span>
