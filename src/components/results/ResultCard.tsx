    ...
    >
      ...
      <div className="px-5 md:px-10 py-8 md:py-12 space-y-10 md:space-y-14">
        <div className="flex justify-between items-start border-b border-slate-100 pb-8">
          <div className="flex items-center gap-6 md:gap-8">
            ...
            <div className="space-y-0.5 text-left min-w-0">
              ...
              <p className="text-[9px] md:text-[11px] font-bold text-slate-400">Audit registry</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
           ...
           <ReportNode label="Attempt accuracy" val={`${attemptAcc}%`} icon={<Zap className="text-orange-500" />} />
        ...
        {subjects.length > 0 && (
          <div className="space-y-4 pt-4">
            <h3 className="text-[9px] md:text-[11px] font-bold text-slate-400 ml-2 text-left">Subject performance</h3>
            <div className="border border-slate-100 rounded-[2rem] overflow-hidden shadow-2xl bg-white">
              <table className="w-full">
                <thead>
                  <tr className="bg-[#0F172A] text-white">
                    <th className="px-6 md:px-10 py-4 font-bold text-left tracking-tight text-[9px] md:text-xs">Subject</th>
                    <th className="px-4 md:px-6 py-4 font-bold text-center tracking-tight text-[9px] md:text-xs">Score</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {subjects.map((s, i) => (
                    <tr key={i} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 md:px-10 py-4 md:py-6 font-bold text-[#0F172A] text-sm md:text-xl tracking-tight text-left">{s.name}</td>
        ...
        <div className="pt-8 md:pt-14 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-14">
          <div className="bg-white border border-slate-100 p-4 rounded-[1.5rem] shadow-xl shrink-0 flex flex-col items-center gap-2">
            ...
            <span className="text-[8px] font-bold text-primary">Verified Question</span>
          </div>
          ...
          <div className="space-y-4 md:space-y-6 flex-1 text-center md:text-left min-w-0 w-full">
            <div className="space-y-2">
               ...
               <div className="min-w-0 px-1">
                  <p className="text-[9px] font-bold text-slate-300 uppercase">Question ID:</p>
            ...
            <p className="text-[10px] md:text-[12px] font-medium text-slate-400 leading-relaxed max-w-xl">
               Generated based on verified recruitment patterns. Authenticity can be audited via the binary QR question or at {webUrlRaw.toLowerCase()}. All performance markers are finalized and synchronized.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={cn(
      ...
      <div className="space-y-0 w-full min-w-0">
        <p className="text-[8px] md:text-[9px] font-bold text-slate-400 truncate">{label}</p>
...
function MetricSummary({ label, val, color }: any) {
  return (
    <div className="text-center space-y-1">
       <p className="text-[8px] md:text-[12px] font-bold text-slate-400">{label}</p>
