...
/**
 * @fileOverview Official Institutional Report Card v10.0 [UPPERCASE & A4].
 * FIXED: Removed blue container/border from Grade icon.
 */
export default function ResultCard({
  ...
  const webUrlRaw = (branding?.websiteUrl || "WWW.CRACKLIX.COM").toUpperCase();
  ...
  return (
    <div id="cracklix-result-card" className="w-[794px] min-h-[1123px] bg-white border border-slate-200 shadow-2xl rounded-none overflow-hidden text-left font-body relative uppercase p-0">
      ...
      <div className="p-16 space-y-12">
        ...
        <div className="flex justify-between items-start border-b-4 border-slate-100 pb-12">
          <div className="flex items-center gap-10">
            ...
            <div className="space-y-2">
              <h2 className="text-6xl font-black text-[#0F172A] tracking-tighter leading-none">{orgName}</h2>
              <p className="text-[14px] font-black text-slate-400 tracking-[0.5em] mt-2">OFFICIAL MERIT PORTAL</p>
            </div>
          </div>
          <div className="text-right space-y-4">
            <Badge className="bg-[#0F172A] text-white border-none font-black text-[12px] px-8 py-3 rounded-full tracking-widest shadow-xl">VERIFIED ATTEMPT</Badge>
        ...
        <div className="grid grid-cols-12 gap-10 items-center">
           <div className="col-span-8 space-y-8">
              <div className="space-y-2">
                 <h1 className="text-6xl font-black text-[#0F172A] tracking-tighter leading-none">{studentName}</h1>
                 <p className="text-primary font-black text-2xl tracking-[0.2em] mt-2">{examTitle}</p>
              </div>
              <div className="flex items-center gap-8 pt-2">
                 <div className="flex items-center gap-3 text-slate-400 font-black text-[12px] tracking-widest">
           ...
           <div className="col-span-4 flex justify-end">
              <div className="h-44 w-44 bg-[#0F172A] rounded-[3rem] shadow-2xl flex flex-col items-center justify-center relative border-8 border-slate-50">
                 <span className="text-[12px] font-black text-slate-500 tracking-widest mb-1">GRADE</span>
        ...
        {subjects.length > 0 && (
          <div className="space-y-8">
            <h3 className="text-sm font-black text-slate-400 tracking-[0.5em] ml-2">SUBJECT PERFORMANCE AUDIT</h3>
            <div className="border-4 border-slate-100 rounded-[3rem] overflow-hidden shadow-2xl bg-white">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#0F172A] text-white">
                    <th className="px-10 py-8 font-black text-left tracking-[0.2em]">SUBJECT HUB</th>
                    <th className="px-6 py-8 font-black text-center tracking-[0.2em]">SCORE</th>
                    <th className="px-10 py-8 font-black text-right tracking-[0.2em]">MASTERY</th>
                  </tr>
                </thead>
                <tbody className="divide-y-4 divide-slate-50">
                  {subjects.map((s, i) => (
                    <tr key={i} className="hover:bg-slate-50 transition-colors">
                      <td className="px-10 py-8 font-black text-[#0F172A] text-2xl tracking-tight">{s.name}</td>
                      <td className="px-6 py-8 text-center font-black text-primary tabular-nums text-4xl">{s.score.toFixed(1)}</td>
                      <td className="px-10 py-8 text-right">
                        <Badge className={cn(
                           "bg-emerald-50 text-emerald-600 border-none font-black text-2xl px-8 py-3 rounded-2xl tabular-nums shadow-sm", 
        ...
        <div className="pt-20 border-t-4 border-slate-100 flex items-center justify-between gap-16">
          <div className="space-y-8 flex-1">
            <div className="flex items-center gap-6">
              ...
              <div className="text-left">
                 ...
                 <p className="text-[14px] font-black text-slate-400 mt-2 tracking-widest">REGISTRY NODE: {resultId}</p>
              </div>
            </div>
            <p className="text-[12px] font-black text-slate-400 tracking-[0.2em] leading-relaxed max-w-xl">
          ...
          <div className="h-64 w-52 bg-white border-4 border-slate-100 p-4 rounded-[3rem] shadow-4xl shrink-0 group hover:scale-105 transition-transform duration-500 flex flex-col items-center justify-center gap-4">
            <img src={qrUrl} alt="VERIFY" className="h-44 w-44 object-contain" crossOrigin="anonymous" />
            <span className="text-[11px] font-black text-primary tracking-widest">VERIFY HUB</span>
          </div>
        </div>
      </div>
    </div>
...
function AnalyticNode({ label, val, icon, highlight }: any) {
  return (
    <div className={cn(
       "p-10 rounded-[3rem] border-4 flex flex-col items-start gap-8 transition-all duration-500 group",
       highlight ? "bg-primary/5 border-primary shadow-xl" : "bg-slate-50/50 border-slate-100 hover:bg-white hover:shadow-2xl"
    )}>
      <div className="h-16 w-16 rounded-2xl bg-white shadow-lg flex items-center justify-center shrink-0 border-2 border-slate-100 group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <div className="space-y-1">
        <p className="text-[12px] font-black text-slate-400 tracking-[0.2em] leading-none mb-1">{label}</p>
        <p className="text-4xl font-black text-[#0F172A] leading-none tabular-nums tracking-tighter">{val}</p>
      </div>
    </div>
  );
}
...
function MetricNode({ label, val, color }: any) {
  return (
    <div className="text-center space-y-2">
       <p className="text-[14px] font-black text-slate-400 tracking-[0.3em] leading-none">{label}</p>
       <p className={cn("text-8xl font-black tabular-nums tracking-tighter", color)}>{val}</p>
    </div>
  );
}
