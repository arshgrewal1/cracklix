    ...
    >
      ...
      <div className="relative z-10 px-20 mt-12 flex gap-12 items-start">
         ...
         <div className="w-[440px] shrink-0">
            <div className="relative w-full h-[780px] bg-slate-800 rounded-[4.5rem] border-[12px] border-slate-900 shadow-5xl overflow-hidden">
               {/* Internal UI simulation */}
               <div className="absolute inset-0 bg-[#F8FAFC] flex flex-col">
                  <div className="h-20 bg-[#2563EB] w-full flex items-center px-8">
                  ...
                  <div className="p-8 space-y-6">
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div className="h-24 bg-white rounded-3xl shadow-sm flex items-center justify-center"><Zap className="text-primary h-8 w-8" /></div>
                        <div className="h-24 bg-white rounded-3xl shadow-sm flex items-center justify-center"><Trophy className="text-primary h-8 w-8" /></div>
                     </div>
                     <div className="h-44 w-full bg-white rounded-[2.5rem] shadow-sm flex flex-col items-center justify-center p-6 gap-4">
                        <BarChart3 className="h-12 w-12 text-[#2563EB]" />
                        <div className="h-2 w-full bg-slate-50 rounded-full overflow-hidden"><div className="h-full bg-primary w-3/4" /></div>
                     </div>
                  </div>
               </div>
               {/* Reflective shine */}
         ...
         <div className="flex-1 space-y-12">
            <div className="space-y-4">
               <h2 className="text-[72px] font-black tracking-tighter leading-[0.85] text-white">
                  Crack Punjab <br/> Exams. 🚀
               </h2>
               <p className="text-2xl text-slate-400 font-medium leading-relaxed">
                  Prepare smarter for PSSSB, PPSC & Punjab Police.
               </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
               ...
               <FeatureRow icon={<Zap className="text-orange-400" />} label="Daily Quiz & Current Affairs" />
               <FeatureRow icon={<Target className="text-rose-400" />} label="Punjab GK & All Subjects" />
               <FeatureRow icon={<Trophy className="text-amber-400" />} label="State Ranking Index" />
            </div>

            <div className="p-10 bg-white/5 border border-white/10 rounded-[3rem] backdrop-blur-xl space-y-6 border-l-4 border-l-primary">
               <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1 text-amber-400">
                     {Array.from({length: 5}).map((_, i) => <Star key={i} className="h-6 w-6 fill-current" />)}
                  </div>
                  <span className="text-slate-400 font-black text-sm uppercase tracking-widest">Top Rated</span>
               </div>
               <p className="text-3xl font-black text-white leading-tight">Join 10,000+ Students <br/> Preparing Daily</p>
            </div>
         </div>
      </div>

      {/* 4. FOOTER CTA HUB */}
      <div className="mt-auto bg-[#2563EB] h-[260px] px-20 flex items-center justify-between">
         <div className="flex items-center gap-8">
            <div className="h-20 w-20 bg-white rounded-[1.5rem] flex items-center justify-center text-[#2563EB] shadow-2xl">
...
function FeatureRow({ icon, label }: any) {
   return (
      <div className="flex items-center gap-6 p-5 bg-white/5 border border-white/10 rounded-2xl group hover:bg-white/10 transition-colors">
