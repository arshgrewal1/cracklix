...
/**
 * @fileOverview Study Anywhere v1.4.
...
export default function AppPreview() {
  const features = [
    ...
    { icon: Zap, label: "Fast engine", desc: "Instant test execution", color: "text-orange-500", bgColor: "bg-orange-50" },
      ...
      <div className="max-w-[1440px] 2xl:max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 space-y-6 md:space-y-16">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-3 text-left px-1">
           <div className="space-y-2">
              <div className="flex items-center gap-3 md:gap-4">
                 ...
                 <h2 className="text-[22px] md:text-4xl font-bold tracking-tight text-[#0F172A]">Study anywhere</h2>
              </div>
              <p className="max-w-2xl text-[14px] md:text-xl font-medium text-slate-500">Experience Punjab's smartest platform on all mobile devices.</p>
           </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8 lg:gap-10">
           {features.map((f, i) => (
            ...
            >
              <Card className="border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 rounded-2xl md:rounded-[3rem] bg-white p-4 md:p-10 lg:p-12 flex flex-col h-full min-h-[220px] md:min-h-[400px] relative overflow-hidden group text-center">
                 ...
                 <div className="flex-1 flex flex-col justify-start text-center space-y-2 md:space-y-6">
                    <h3 className="text-[15px] md:text-2xl font-bold tracking-tight text-[#0F172A]">
                       {f.label}
                    </h3>
                    <p className="text-[12px] md:text-base font-medium text-slate-400 leading-snug line-clamp-3 tracking-tight">
                       {f.desc}
                    </p>
                 </div>

                 <div className="mt-auto shrink-0 pt-6 md:pt-10">
                    <Button asChild className="w-full h-11 md:h-14 lg:h-16 rounded-full bg-primary hover:bg-blue-700 text-white transition-all font-bold text-[14px] md:text-[15px] tracking-tight border-none shadow-xl active:scale-95">
        ...
        <div className="w-full pt-6 md:pt-12">
           ...
           >
              <Card className="bg-[#0B1528] rounded-[2rem] md:rounded-[4rem] p-8 md:p-24 text-white relative overflow-hidden shadow-5xl group border border-white/5 mx-1 text-center lg:text-left">
                 ...
                 <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10 md:gap-16">
                    <div className="space-y-6 md:space-y-12 flex-1">
                       <h3 className="text-3xl md:text-7xl font-black tracking-tighter leading-[0.9] text-white">Elite hub</h3>
                       <div className="flex flex-wrap justify-center lg:justify-start gap-x-6 gap-y-4 text-xs md:text-xl font-bold text-slate-400 tracking-tight">
                    ...
                    <Button asChild className="w-full lg:w-auto h-16 md:h-24 px-12 md:px-24 bg-primary hover:bg-blue-700 text-white font-bold text-[14px] md:text-lg shadow-4xl transition-all border-none active:scale-95 shrink-0 rounded-full">
