...
'use client';
/**
 * @fileOverview Institutional Deterministic Ingestion Hub v41.0.
...
function parseReasoning(rawText: string, metadata: any) {
  ...
  lines.forEach((line) => {
    ...
    // Metadata Filter: Skip noise lines inside the reasoning loop
    const isNoise = NOISE_PATTERNS.some(pattern => pattern.test(trimmed));
    if (isNoise) return;
  ...
  // FINAL QUESTION COMMIT: Finalize the last question in the document stream
  if (currentQ) finalizeQuestion();
