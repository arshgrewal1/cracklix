/**
 * @fileOverview Institutional Deterministic Ingestion Hub v18.5.
 * FIXED: Purged autogenerated labels (Mathematics Number Line, etc.) from Reasoning Parser.
 * FIXED: Numbers/Sequences stowed in English field; Local script isolated to local field.
 * RULES: English ends immediately before the first detected local script character.
...
export function parseBulkQuestions(rawText: string, metadata: any) {
  ...
  // Support for 15+ dynamic question start formats (Q1, Q.1, Question-1, 1., 1), etc)
...
/**
 * REASONING PARSER v2.1
 * Multiline awareness, zero autogenerated labels, strict English/Local script isolation.
 */
function parseReasoning(block: string, q: any, secondaryLang: string) {
  ...
  lines.forEach((line, idx) => {
    ...
    // Transition detections (Option format support A), (A), A., ①, etc)
    if (trimmedLine.match(/^\(?[A-E][\)\.\s:]/i) || trimmedLine.match(/^[①②③④❶❷❸❹]/)) {
      state = 'OPTIONS';
    ...
    if (state === 'QUESTION') {
      let text = idx === 0 ? line.replace(qMarkerRegex, '') : line;
      
      // RULE: English question ends immediately before the first local script character.
      // Numbers/Series remain in English field (Rule 5).
      if (scriptRegex.test(text)) {
        // Line contains local script characters
        localQLines.push(text.trim());
      } else if (text.trim()) {
        // Line is pure English or Number Series
        englishQLines.push(text.trim());
      }
      ...
      // Circled marker mapping
      Object.entries(circleMap).forEach(([char, label]) => {
         if (trimmedLine.startsWith(char)) {
            ...
            if (scriptRegex.test(content)) {
               ...
               q[`option${label}English`] = content;
            }
         }
      });
...
function parseDeterministicEnglish(block: string, q: any) {
  ...
  lines.forEach((line, idx) => {
    ...
    if (section === 'QUESTION') {
      ...
      const circleMap: Record<string, string> = { '①': 'A', '②': 'B', '③': 'C', '④': 'D', '❶': 'A', '❷': 'B', '❸': 'C', '❹': 'D' };
      Object.entries(circleMap).forEach(([char, label]) => {
         if (trimmedLine.startsWith(char)) q[`option${label}English`] = trimmedLine.substring(1).trim();
      });
...
function parseDeterministicBilingual(block: string, q: any, secondaryLang: string) {
  ...
  lines.forEach((line, idx) => {
    ...
    if (section === 'QUESTION') {
      const text = idx === 0 ? line.replace(/^(?:Q\d+|Question\s*\d+|\d+\.|\d+\))(?:[\.\s:]|$)\s*/i, '') : line;
      if (text.trim()) {
