'use client';
/**
 * @fileOverview Institutional Deterministic Ingestion Hub v35.0.
 * FIXED: Purged horizontal separators and decorative noise (dots, dashes) from document stream.
 * LOCKED: Current Affairs, Mathematics, and Simple Bilingual parsers remain production-ready.
 */
...
const NOISE_PATTERNS = [
  ...
  /^[-\s\.\*_=]{3,}$/, // FIXED: Removed horizontal lines (dashes, dots, stars, equals)
  /^\s*[\.·,]{1,}\s*$/ // FIXED: Removed standalone punctuation noise lines (dots/bindis/commas)
];
...
export function preprocessText(text: string): string {
  ...
  const cleanLines = lines.filter(line => {
    ...
    if (!trimmed) return true; // PRESERVE BLANK LINES FOR FIDELITY
 ...
 * FIXED: Character-perfect fidelity accumulation.
 */
