...
/**
 * @fileOverview Hardened Operational Node Monitor v5.0.
 * FIXED: Explicitly typed ReactElement in cloneElement for React 19 / TS 5.
 */
...
function HealthCard({ label, value, status, icon, color }: { label: string, value: string | number, status: string, icon: React.ReactNode, color: string }) {
   return (
      <Card className="border-none shadow-lg bg-white p-5 md:p-8 rounded-2xl md:rounded-[2rem] relative overflow-hidden group border border-slate-50 text-left hover:translate-y-[-4px]">
         <div className="absolute top-0 right-0 p-4 md:p-6 opacity-5 group-hover:scale-110 transition-transform">
            {isValidElement(icon) ? cloneElement(icon as ReactElement<{ className?: string }>, { className: cn("h-5 w-5 md:h-8 md:w-8", color) }) : null}
         </div>
