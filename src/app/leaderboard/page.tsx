...
import { Trophy, Search, Activity, Zap, Star, Medal, Target, ChevronRight, X, Users, Layout, Timer, Loader2, ShieldCheck } from "lucide-react"
         ...
         <section className="space-y-4 px-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
               <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground font-medium text-sm md:text-xl">Rankings based on verified best attempts. Refresh occurs live upon submission.</p>
               </div>
               <div className="flex items-center gap-4 bg-card border border-border p-4 rounded-2xl shadow-xl shrink-0 mx-auto md:mx-0">
                  ...
                  <div className="text-left">
                     ...
                     <p className="text-2xl font-black text-foreground tabular-nums leading-none mt-1">{rawMeritList?.length || 0}</p>
                  </div>
               </div>
            </div>
         </section>
            ...
            <AnimatePresence mode="popLayout">
               {loadingList ? (
                  Array.from({ length: 8 }).map((_, i) => (
                     <div key={i} className="h-[90px] w-full bg-muted rounded-2xl border border-border animate-pulse" />
                  ))
               ) : filteredList.length > 0 ? (
                  (searchTerm ? filteredList : listItems).map((entry, idx) => {
                     const isCurrent = user?.uid === entry.userId || user?.uid === entry.uid || user?.uid === entry.id;
                     return (
                        <motion.div key={entry.userId || entry.uid || entry.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: idx * 0.02 }}>
                           <Card className={cn("border border-border shadow-sm hover:shadow-4xl transition-all duration-300 rounded-2xl bg-card group overflow-hidden", isCurrent && "ring-2 ring-primary bg-primary/[0.02] shadow-xl")}>
