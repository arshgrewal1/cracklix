         ...
         <section className="space-y-4 px-1">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
               <div className="space-y-2">
                  ...
                  <h1 className="text-3xl md:text-6xl font-black text-[#0F172A] tracking-tighter">
         ...
         <div className="sticky top-[84px] md:top-[116px] z-[45] bg-[#F8FAFC]/95 backdrop-blur-xl -mx-4 px-4 py-6 border-b border-slate-100">
            <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-4 items-center">
               ...
               <Link href="/mocks" className="w-full md:w-auto">
                  <Button variant="outline" className="w-full md:w-auto h-14 md:h-16 rounded-2xl px-8 font-bold text-[10px] border-2">All tests</Button>
               </Link>
            </div>
         </div>
                  ...
                  (searchTerm ? filteredList : listItems).map((entry, idx) => {
                     ...
                     return (
                        <motion.div key={entry.userId || entry.uid} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: idx * 0.02 }}>
                           <Card className={cn("border border-slate-100 shadow-sm hover:shadow-4xl transition-all duration-300 rounded-2xl bg-white group overflow-hidden", isCurrent && "ring-2 ring-primary bg-primary/[0.02] shadow-xl")}>
                              <CardContent className="p-0 flex items-center h-[72px] md:h-[90px]">
                                 <div className="w-12 md:w-20 flex items-center justify-center shrink-0">
                                    <span className="text-lg md:text-3xl font-black text-slate-200 group-hover:text-primary transition-colors tabular-nums">#{searchTerm ? idx + 1 : idx + 4}</span>
                                 </div>
                                 <div className="flex-1 flex items-center gap-4 min-w-0 pr-4">
                                    <StudentAvatar profile={{ name: entry.userName || entry.displayName, photoURL: entry.photoURL, gender: entry.gender }} className="h-10 w-10 md:h-14 md:w-14 rounded-xl shrink-0 shadow-inner bg-slate-50" />
                                    <div className="min-w-0 flex-1 text-left">
                                       <h4 className="font-bold text-sm md:text-xl text-[#0F172A] truncate leading-tight group-hover:text-primary transition-colors">{entry.userName || entry.displayName}</h4>
...
function PodiumCard({ rank, data, order, isMain, formatTime, currentUser }: any) {
   if (!data) return <div className={cn("bg-white border border-dashed border-slate-200 rounded-[3rem] p-4 h-64 flex items-center justify-center opacity-40", order)}><Trophy className="h-10 w-10 text-slate-200" /></div>;
   ...
   return (
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: rank * 0.1 }} className={cn("flex", order)}>
         <Card className={cn("border-none shadow-xl transition-all duration-500 rounded-[2rem] p-5 md:p-8 flex flex-col items-center text-center group hover:-translate-y-1 relative overflow-hidden w-full", isMain ? "bg-[#0F172A] text-white ring-8 ring-primary/10 scale-[1.02] z-10" : "bg-white text-[#0F172A]", isCurrent && !isMain && "ring-4 ring-primary/20")}>
            ...
            <div className="space-y-4 relative z-10 w-full flex flex-col items-center">
               <div className="relative inline-block">
                  <StudentAvatar profile={{ name: data.userName || data.displayName, photoURL: data.photoURL, gender: data.gender }} className={cn("rounded-[1.5rem] border-[3px] shadow-xl transition-all group-hover:scale-105", isMain ? "h-20 w-20 md:h-24 md:w-24 border-primary/20" : "h-16 w-16 md:h-20 md:w-20 border-white")} />
               ...
               <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-500/10 w-full">
                  ...
                  <div className="text-center space-y-1">
                     ...
                     <p className={cn("text-base md:text-2xl font-black tabular-nums tracking-tighter", isMain ? "text-emerald-400" : "text-[#0F172A]")}>{data.accuracy || 0}%</p>
                  </div>
               </div>
               <div className="pt-1"><p className="text-[8px] font-bold text-slate-400 flex items-center justify-center gap-2"><Timer className="h-2.5 w-2.5" /> {formatTime(data.timeTaken || 0)}</p></div>
            </div>
         </Card>
      </motion.div>
   )
