        ...
        <Tabs defaultValue="SOLUTIONS" className="space-y-6 md:space-y-10">
           ...
           <TabsContent value="SOLUTIONS" className="space-y-6 md:space-y-10 animate-in fade-in slide-in-from-bottom-3 duration-700">
              {filteredQuestions.length > 0 ? filteredQuestions.map((q) => {
                 ...
                 return (
                    <Card key={q.id} className="border-none shadow-xl rounded-[2rem] md:rounded-[3rem] overflow-hidden bg-white text-left relative group border border-slate-100">
                       <div className={cn("absolute top-0 left-0 w-2 h-full transition-all duration-500", isCorrect ? 'bg-emerald-500' : isSkipped ? 'bg-slate-200' : 'bg-rose-500')} />
                       <CardContent className="p-6 md:p-14 lg:p-20">
                          <div className="flex items-center justify-between mb-8 md:mb-12">
                             <div className="flex items-center gap-4">
                                <span className={cn("h-10 w-10 md:h-14 md:w-14 rounded-2xl flex items-center justify-center font-black text-base md:text-2xl shadow-inner", isCorrect ? "bg-emerald-50 text-emerald-600" : isSkipped ? "bg-slate-50 text-slate-400" : "bg-rose-50 text-rose-600")}>{q.index + 1}</span>
                                <Badge variant="outline" className="border-slate-100 text-slate-400 uppercase text-[9px] md:text-[11px] font-black px-3 py-1 rounded-lg">{(q.subjectId || "GK").toUpperCase()}</Badge>
                             </div>
                             {isCorrect ? (
                                <Badge className="bg-emerald-50 text-emerald-600 border-none font-black text-[9px] md:text-[11px] uppercase px-4 py-1.5 rounded-full shadow-lg">CORRECT</Badge>
                             ) : isSkipped ? (
                                <Badge className="bg-slate-50 text-slate-400 border-none font-black text-[9px] md:text-[11px] uppercase px-4 py-1.5 rounded-full">SKIPPED</Badge>
                             ) : (
                                <Badge className="bg-rose-50 text-rose-600 border-none font-black text-[9px] md:text-[11px] uppercase px-4 py-1.5 rounded-full shadow-lg">INCORRECT</Badge>
                             )}
                          </div>
                          <QuestionRenderer 
                            question={q} 
                 ...
                 <div className="py-32 text-center opacity-20 flex flex-col items-center gap-6">
                    ...
                    <p className="font-headline font-black text-2xl uppercase tracking-[0.4em]">No Results Filtered</p>
                 </div>
              )}
           </TabsContent>

           <TabsContent value="TOPPER" className="space-y-6 md:space-y-10 animate-in fade-in slide-in-from-bottom-3 duration-700">
              <Card className="border-none shadow-3xl rounded-[2.5rem] md:rounded-[4rem] bg-white p-6 md:p-14 text-left border border-slate-100">
                 <div className="space-y-6 md:space-y-10">
                    <div className="p-5 md:p-8 bg-slate-50 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-100 flex items-center justify-between mb-8 md:mb-12">
                       <div className="flex items-center gap-4">
                          <Users className="h-6 w-6 text-primary" />
                          <p className="text-[10px] md:text-[14px] font-black uppercase text-slate-500 tracking-widest">Global Aspirant Node: <span className="text-[#0F172A]">{merit.total} Active Verticals</span></p>
                       </div>
                    </div>
