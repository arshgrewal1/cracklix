                    ...
                    {profileLoading ? (
                      ...
                      <>
                        <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-5">
                          ...
                          <Badge className={cn("border-none text-[8px] md:text-[12px] font-black px-3 py-1 rounded-full shadow-2xl shrink-0", profile?.status === 'Free' ? "bg-white/10 text-slate-300" : "bg-primary text-white")}>{profile?.status || 'Free'} pass</Badge>
                        </div>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-12">
              <div className="lg:col-span-8 space-y-4 md:space-y-10">
                    ...
                    <CardContent className="p-0">
                       <div className="divide-y divide-border">
                          {resultsLoading ? (
                             ...
                             <div className="py-20 text-center opacity-20 italic font-black text-sm">No attempts recorded</div>
                          )}
                       </div>
                    </CardContent>
                 </Card>
                 ...
                 <Card className="border-none shadow-3xl rounded-[2.5rem] bg-card p-5 md:p-14 space-y-6 md:space-y-12 text-left border border-border">
                    <div className="flex items-center justify-between border-b border-border pb-4 md:pb-10"><h3 className="font-black text-lg md:text-3xl flex items-center gap-3 md:gap-6 text-foreground tracking-tighter"><UserIcon className="h-5 w-5 md:h-8 md:w-8 text-primary" /> Profile details</h3></div>
              ...
              <div className="lg:col-span-4 space-y-4 md:space-y-10">
                 <Card className="border-none shadow-2xl rounded-[2.5rem] bg-card p-5 md:p-12 space-y-6 md:space-y-10 border border-border">
                    <h3 className="text-[10px] md:text-[12px] font-black tracking-tight text-muted-foreground">Settings hub</h3>
                    <div className="space-y-3 md:space-y-5">
                       <Button onClick={() => setIsEditing(true)} className="w-full h-12 md:h-18 bg-blue-600 hover:bg-blue-700 text-white font-black text-[9px] md:text-[12px] tracking-tight shadow-xl transition-all active:scale-95 border-none gap-2"><Edit className="h-4 w-4 text-white" /> Edit profile</Button>
                       <Button asChild variant="outline" className="w-full h-12 md:h-18 rounded-full font-black text-[9px] md:text-[12px] tracking-tight shadow-sm transition-all active:scale-95 border-2 gap-2"><Link href="/pass"><Gem className="h-4 w-4 text-primary" /> Elite pass hub</Link></Button>
                    </div>

                    <div className="pt-8 md:pt-12 border-t border-border space-y-4">
                       <p className="text-[8px] md:text-[10px] font-black text-muted-foreground tracking-tight">Account persistence</p>
                       <Button onClick={() => setIsDeleting(true)} variant="ghost" className="w-full h-10 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-xl font-black text-[8px] tracking-tight transition-all gap-2"><Trash2 className="h-3.5 w-3.5" /> Delete my account</Button>
                    </div>
                 </Card>
                 ...
                 <Card className="border-none shadow-xl rounded-[2.5rem] bg-[#0F172A] text-white p-8 md:p-10 space-y-8 relative overflow-hidden group border border-white/5">
                    ...
                    <div className="relative z-10 space-y-6">
                       <div className="space-y-1">
                          ...
                          <p className="text-[10px] font-bold text-slate-500 tracking-tight">Mastery records</p>
                       </div>
                       <div className="h-40 bg-white/5 rounded-3xl border border-white/10 flex flex-col items-center justify-center text-center p-6 gap-3">
                          ...
                          <p className="text-xs font-bold text-slate-400 leading-relaxed">No certificates awarded yet. Reach Rank #1 to earn.</p>
                       </div>
                    </div>
                 </Card>
              </div>
         ...
         <DialogContent className="sm:max-w-2xl w-[95vw] max-h-[95vh] bg-card rounded-3xl md:rounded-[3rem] border-none shadow-5xl p-0 overflow-hidden text-left flex flex-col">
            ...
            <DialogHeader className="p-6 md:p-14 pb-3 md:pb-6 shrink-0">
               <div className="flex justify-between items-center">
                  <DialogTitle className="text-xl md:text-4xl font-black text-foreground flex items-center gap-3 md:gap-6 tracking-tighter">
            ...
            <div className="px-6 md:px-14 pb-6 md:pb-14 space-y-4 md:space-y-8 overflow-y-auto custom-scrollbar flex-1">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                  <div className="space-y-1.5 text-left"><Label className="text-[9px] font-black text-muted-foreground ml-1">Full name</Label><Input value={editForm?.name || ""} onChange={e => setEditForm((prev: any) => ({...prev, name: e.target.value}))} className="h-12 md:h-16 rounded-xl bg-muted border-none font-bold px-5" /></div>
                  <div className="space-y-1.5 text-left"><Label className="text-[9px] font-black text-muted-foreground ml-1">Email address</Label><Input type="email" value={editForm?.email || ""} onChange={e => setEditForm((prev: any) => ({...prev, email: e.target.value}))} className="h-12 md:h-16 rounded-xl bg-muted border-none font-bold px-5" /></div>
               </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                  <div className="space-y-1.5 text-left"><Label className="text-[9px] font-black text-muted-foreground ml-1">Date of birth</Label><Input type="date" value={editForm?.dob || ""} onChange={e => setEditForm((prev: any) => ({...prev, dob: e.target.value}))} className="h-12 md:h-16 rounded-xl bg-muted border-none font-bold px-5" /></div>
                  <div className="space-y-1.5 text-left"><Label className="text-[9px] font-black text-muted-foreground ml-1">Target exam hub</Label><select value={editForm?.targetExam || ""} onChange={e => setEditForm((prev: any) => ({...prev, targetExam: e.target.value}))} className="w-full h-12 md:h-16 rounded-xl bg-muted border-none font-bold px-5 outline-none text-foreground"><option value="PSSSB">PSSSB</option><option value="PPSC">PPSC</option><option value="Punjab Police">Punjab Police</option><option value="Army">Indian Army</option><option value="High Court">High Court</option></select></div>
               </div>
               <div className="space-y-1.5 text-left">
                  <Label className="text-[9px] font-black text-muted-foreground ml-1">Mobile number</Label>
                  <div className="relative">
               ...
               <div className="space-y-1.5 text-left">
                  <Label className="text-[9px] font-black text-muted-foreground ml-1">Home address</Label>
            ...
            <DialogFooter className="p-6 md:p-14 pt-3 bg-muted border-t border-border flex flex-row gap-4 items-center justify-between">
               <Button variant="ghost" onClick={() => setIsEditing(false)} className="h-12 px-6 font-black text-[9px] text-muted-foreground border-none">Cancel</Button>
               <Button onClick={handleUpdateProfile} disabled={isSaving} className="bg-primary hover:bg-blue-700 text-white h-12 px-8 rounded-full font-black text-[10px] flex-1 shadow-3xl transition-all active:scale-95 gap-2">{isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Save details</Button>
            </DialogFooter>
         </DialogContent>
      </Dialog>
         ...
         <DialogContent className="max-w-md w-[95vw] bg-card rounded-3xl p-8 shadow-5xl border border-border text-center">
            <div className="space-y-6">
               ...
               <div className="space-y-2">
                  <DialogTitle className="text-2xl font-black text-rose-600 tracking-tighter">Delete account</DialogTitle>
               ...
               <div className="flex gap-4 pt-2">
                  <Button variant="ghost" onClick={() => setIsDeleting(false)} className="flex-1 rounded-xl h-12 font-black text-[9px] text-muted-foreground">Cancel</Button>
                  <Button onClick={handleDeleteAccount} disabled={deleteConfirm !== 'DELETE' || isSaving} className="flex-1 bg-rose-600 hover:bg-rose-700 text-white rounded-xl h-12 font-black text-[9px] shadow-lg border-none active:scale-95 transition-all">Authorize purge</Button>
               </div>
            </div>
         </DialogContent>
      </Dialog>
    ...
    <Card className={cn("border-none shadow-xl rounded-2xl md:rounded-[2rem] p-4 md:p-6 bg-card group hover:translate-y-[-4px] transition-all duration-500 border border-border", className)}>
      <div className="flex flex-col items-center text-center gap-3">
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-[7px] md:text-[9px] font-black tracking-tight text-muted-foreground mt-1">{label}</p>
        </div>
      </div>
    </Card>
   )
