           ...
           <div className="container mx-auto px-4 md:px-6 max-w-6xl pt-6 md:pt-16 pb-12 md:pb-20">
              <div className="flex flex-col md:flex-row items-center md:items-end gap-5 md:gap-10 relative z-10">
                 <div className="relative group shrink-0">
                    <StudentAvatar 
                      profile={profile} 
                      className="h-20 w-20 md:h-32 md:w-32 border-[3px] border-white/10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl relative bg-[#0F172A]" 
                    />
                 ...
                 <div className="flex-1 space-y-2 md:space-y-3 min-w-0 text-center md:text-left">
                    <div className="flex flex-col md:flex-row items-center gap-2 md:gap-3">
                       <h1 className="text-xl md:text-5xl font-headline font-black text-white uppercase tracking-tight truncate max-w-full">
                          {profile.name}
                       </h1>
                       <Badge className={cn(
                          "border-none text-[8px] md:text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded shadow-lg",
                 ...
                 <div className="shrink-0 w-full md:w-auto pt-3 md:pt-0 flex flex-col sm:flex-row gap-2 md:gap-3">
                    <Button onClick={() => setIsEditing(true)} className="h-10 md:h-14 px-6 md:px-8 bg-white/10 hover:bg-white/20 text-white border border-white/10 rounded-xl font-black uppercase text-[9px] md:text-[10px] tracking-widest gap-2 shadow-2xl">
                       <Edit className="h-3.5 w-3.5 text-primary" /> Edit
                    </Button>
                    <Button asChild className="h-10 md:h-14 px-6 md:px-8 bg-primary hover:bg-orange-600 text-white rounded-xl font-black uppercase text-[9px] md:text-[10px] tracking-widest gap-2 shadow-2xl border-none">
                       <Link href="/pass"><Gem className="h-3.5 w-3.5" /> Upgrade</Link>
                    </Button>
                 </div>
              </div>
           </div>
              ...
              <div className="lg:col-span-8 space-y-5 md:space-y-8">
                 ...
                 <Card className="border-none shadow-xl rounded-[2rem] md:rounded-[2.5rem] bg-white p-6 md:p-10 space-y-6 md:space-y-8">
                    <div className="flex items-center justify-between border-b border-slate-50 pb-4 md:pb-6">
                       <h3 className="font-headline font-black text-lg md:text-2xl uppercase flex items-center gap-3 md:gap-4 text-[#0F172A]">
                          <UserIcon className="h-5 w-5 md:h-6 md:w-6 text-primary" /> My Profile
                       </h3>
                    </div>
                 ...
                 <Card className="border-none shadow-xl rounded-[2rem] md:rounded-[2.5rem] overflow-hidden bg-white">
                    <CardHeader className="p-6 md:p-10 border-b border-slate-50 flex flex-row items-center justify-between">
                       <h3 className="text-[11px] md:text-lg font-black uppercase tracking-tight text-[#0F172A] flex items-center gap-2">
                          <History className="h-4 w-4 md:h-5 md:w-5 text-primary" /> History
                       </h3>
                       <Button asChild variant="ghost" className="h-8 text-[8px] md:text-[9px] font-black uppercase tracking-widest text-primary gap-1">
                    ...
                    <CardContent className="p-0">
                       <div className="divide-y divide-slate-50">
                          {resultsLoading ? (
                             Array.from({ length: 2 }).map((_, i) => <div key={i} className="p-6"><Skeleton className="h-10 w-full rounded-xl" /></div>)
                          ) : results.length > 0 ? (
                             results.slice(0, 3).map((r: any) => (
                                <Link key={r.id} href={`/results/${r.mockId}`} className="p-5 md:p-10 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                                   <div className="flex items-center gap-4 md:gap-6 min-w-0">
                                      ...
                                      <div className="min-w-0 space-y-1">
                                         <p className="font-black text-[#0B1528] text-xs md:text-lg uppercase truncate leading-none">{r.mockTitle}</p>
                                         <p className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                            {new Date(r.timestamp).toLocaleDateString()} • {r.accuracy}%
                                         </p>
                                      </div>
                                   </div>
                             ...
                             <div className="p-12 text-center text-slate-300 font-bold uppercase text-[10px]">No activity found.</div>
                          )}
                       </div>
                    </CardContent>
                 </Card>
              ...
              <div className="lg:col-span-4 space-y-5 md:space-y-8">
                 <Card className="border-none shadow-xl rounded-[2rem] bg-white p-6 md:p-8 space-y-6 md:space-y-8">
                    ...
                    <div className="space-y-3 md:space-y-4">
                       <Button asChild className="w-full h-11 md:h-14 bg-[#0F172A] hover:bg-black text-white rounded-xl font-black uppercase text-[9px] md:text-[10px] tracking-widest shadow-xl gap-2">
                          <Link href="/pass"><CreditCard className="h-4 w-4 text-primary" /> Unlock Elite Pass</Link>
                       </Button>
                       <Button asChild variant="outline" className="w-full h-11 md:h-14 border-slate-100 bg-slate-50/50 rounded-xl font-black uppercase text-[9px] md:text-[10px] tracking-widest text-slate-600 gap-2">
                          <Link href="/dashboard"><History className="h-4 w-4" /> Global Report</Link>
                       </Button>
                    </div>
                 </Card>
                 ...
                 <div className="bg-primary rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-10 text-white space-y-5 relative overflow-hidden group">
                    <div 
                      className="absolute top-0 right-0 p-6 opacity-10 rotate-12 group-hover:scale-110 transition-transform"
                      style={{ transitionDuration: '2000ms' }}
                    >
                       <Award className="h-32 w-32 md:h-40 md:w-40" />
                    </div>
                    <div className="relative z-10 space-y-3 md:space-y-4">
                       ...
                       <Button asChild className="w-full mt-4 md:mt-8 bg-white text-primary hover:bg-slate-50 font-black h-10 md:h-12 rounded-xl text-[9px] md:text-[10px] uppercase shadow-lg">
         ...
         <DialogContent className="sm:max-w-xl w-[95vw] max-h-[90vh] bg-white rounded-[2.5rem] border-none shadow-5xl p-0 overflow-hidden text-left flex flex-col">
            ...
            <div className="px-6 md:px-8 pb-6 md:pb-8 space-y-4 md:space-y-5 overflow-y-auto custom-scrollbar flex-1">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  <div className="space-y-1.5 text-left">
                     ...
                     <Input value={editForm?.name || ""} onChange={e => setEditForm({...editForm, name: e.target.value})} className="h-10 md:h-11 rounded-xl bg-slate-50 border-none font-bold text-xs md:text-sm" />
                  </div>
                  <div className="space-y-1.5 text-left">
                     <Label className="text-[9px] font-black uppercase text-slate-500 ml-1">Email Address</Label>
                     <Input type="email" value={editForm?.email || ""} onChange={e => setEditForm({...editForm, email: e.target.value})} className="h-10 md:h-11 rounded-xl bg-slate-50 border-none font-bold text-xs md:text-sm" />
                  </div>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  <div className="space-y-1.5 text-left">
                     ...
                     <Input type="date" value={editForm?.dob || ""} onChange={e => setEditForm({...editForm, dob: e.target.value})} className="h-10 md:h-11 rounded-xl bg-slate-50 border-none font-bold text-xs md:text-sm" />
                  </div>
                  <div className="space-y-1.5 text-left">
                     ...
                     <select 
                        ...
                        className="w-full h-10 md:h-11 rounded-xl bg-slate-50 border-none font-bold text-xs md:text-sm px-4 outline-none"
                     >
               ...
               <div className="space-y-1.5 text-left">
                  ...
                  <div className="relative">
                    ...
                    <Input 
                      ...
                      className="h-10 md:h-11 pl-12 md:pl-14 rounded-xl bg-slate-50 border-none font-black text-base md:text-lg tracking-wider" 
            ...
            <DialogFooter className="p-5 md:p-8 pt-3 md:pt-6 bg-slate-50 border-t border-slate-100 shrink-0 flex flex-row gap-3 md:gap-4 items-center justify-between">
               ...
               <Button 
                ...
                disabled={isSaving} 
                className="bg-primary hover:bg-orange-600 text-white h-10 md:h-14 px-6 md:px-12 rounded-xl font-black uppercase text-[9px] md:text-[11px] tracking-[0.2em] flex-1 shadow-xl transition-all active:scale-95 gap-2"
               >
                  {isSaving ? <Loader2 className="h-3 w-3 animate-spin" /> : <Save className="h-3 w-3" />} Save Profile
               </Button>
            </DialogFooter>
         </DialogContent>
      </Dialog>
...
function HeaderInfo({ icon, text }: { icon: React.ReactNode, text: string }) {
   return (
      <div className="flex items-center gap-2 text-white/60 font-bold uppercase text-[9px] md:text-[11px] tracking-tight">
         <span className="shrink-0">{icon}</span>
         <span className="truncate max-w-[180px] md:max-w-[240px]">{text || 'Not Added'}</span>
      </div>
   )
}
...
function StatsCard({ icon, label, value, color, bgColor, className }: any) {
   return (
      <Card className={cn("border-none shadow-lg rounded-[1.5rem] p-4 md:p-8 bg-white group hover:translate-y-[-4px] transition-all", className)}>
         <div className="flex flex-col gap-3 md:gap-4">
            <div className={cn("h-8 w-8 md:h-10 md:w-10 rounded-xl flex items-center justify-center transition-all group-hover:scale-110 shadow-sm", bgColor)}>
               {React.isValidElement(icon) ? React.cloneElement(icon as React.ReactElement, { className: cn("h-4 w-4 md:h-5 md:w-5", color) }) : icon}
            </div>
            <div className="space-y-0.5 md:space-y-1 text-left">
               <p className="text-[7px] md:text-[9px] font-black uppercase tracking-widest text-slate-400">{label}</p>
               <p className={cn("text-lg md:text-4xl font-headline font-black leading-none", color)}>{value}</p>
            </div>
         </div>
      </Card>
   )
...
function ProfileDataNode({ icon, label, value, colSpan = 1 }: any) {
   return (
      <div className={cn("flex items-start gap-4 md:gap-5", colSpan > 1 ? "md:col-span-2" : "")}>
         <div className="h-10 w-10 md:h-12 md:w-12 rounded-[1rem] md:rounded-2xl bg-slate-50 flex items-center justify-center shrink-0 shadow-inner">
            {React.cloneElement(icon, { className: "h-4 w-4 md:h-5 md:w-5" })}
         </div>
         <div className="min-w-0 space-y-0.5 md:space-y-1 text-left">
            <p className="text-[7px] md:text-[9px] font-black uppercase tracking-widest text-slate-400">{label}</p>
            <p className="text-xs md:text-base font-bold text-[#0F172A] leading-relaxed break-words uppercase">{value}</p>
         </div>
      </div>
   )
}
