...
/**
 * @fileOverview Institutional Hub Explorer v9.0.
 * DESIGN: Compact mobile header, 64px logo, and text-4xl mobile headlines.
 */
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 font-body text-left">
      ...
      <section className="bg-white border-b border-slate-100 py-10 md:py-20 overflow-hidden relative">
         <div className="absolute top-0 right-0 p-12 opacity-[0.03]"><GraduationCap className="h-64 w-64" /></div>
         <div className="container mx-auto px-4 max-w-7xl relative z-10">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-10">
               <div className="h-16 w-16 md:h-32 md:w-32 rounded-2xl md:rounded-[2.5rem] bg-slate-50 border-2 md:border-4 border-white shadow-xl flex items-center justify-center overflow-hidden shrink-0">
                  {hub?.iconUrl && !failedImages['hub'] ? (
                    <img src={hub.iconUrl} className="h-full w-full object-contain p-2 md:p-4" alt="Logo" referrerPolicy="no-referrer" onError={() => setFailedImages(p => ({...p, 'hub': true}))} />
                  ) : (
               ...
               <div className="space-y-2 md:space-y-4 text-center md:text-left">
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                     <Badge className="bg-primary text-white border-none font-black px-4 py-1 rounded-xl text-[9px] tracking-widest">{hub?.abbreviation} EXAMS</Badge>
                     <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">OFFICIAL VERTICALS</span>
                  </div>
                  <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-[#0F172A] uppercase tracking-tight leading-[0.95] max-w-2xl">{hub?.name}</h1>
               </div>
            </div>
         </div>
      </section>
                          ...
                          <div className="flex justify-between items-start mb-8">
                             <div className="h-14 w-14 md:h-18 md:w-18 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0 shadow-inner">
                                {effectiveLogo && !failedImages[exam.id] ? (
                                   <img src={effectiveLogo} className="w-full h-full object-contain p-2" alt="Logo" referrerPolicy="no-referrer" onError={() => setFailedImages(p => ({ ...p, [exam.id]: true }))} />
                                ) : (
