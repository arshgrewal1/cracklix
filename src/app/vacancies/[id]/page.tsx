...
/**
 * @fileOverview Enterprise Recruitment Intelligence Portal v5.4.
 * COMPACT: Reduced padding and sizes for optimized layout.
 * FIXED: Validated JSX tag closure for Button and handled async params.
 */
...
export default function VacancyDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-white font-body text-left selection:bg-primary/10 flex flex-col break-words">
      ...
      <main className="flex-1 w-full max-w-[1280px] mx-auto px-4 md:px-8 py-6 md:py-12 space-y-8 md:space-y-16">
         
         <section className="bg-[#0F172A] rounded-[2rem] md:rounded-[3rem] shadow-5xl overflow-hidden relative group text-white">
            <div className="absolute top-0 right-0 w-1/2 h-full bg-primary/10 blur-[120px] rounded-full pointer-events-none" />
            
            <div className="relative z-10 p-6 md:p-12 flex flex-col lg:flex-row gap-8 md:gap-14 items-center">
               <div className="relative shrink-0 transition-transform duration-700 group-hover:scale-105">
                  <AuthorityLogo boardId={vacancy.board} size="lg" className="h-24 w-24 md:h-48 md:w-48 border-[6px] border-white/5 shadow-5xl bg-white/5 backdrop-blur-3xl" />
                  <div className="absolute -bottom-2 -right-2 h-10 w-10 md:h-14 md:w-14 bg-emerald-500 rounded-xl border-[4px] border-[#0F172A] shadow-2xl flex items-center justify-center text-white"><ShieldCheck className="h-6 w-6 md:h-8 md:w-8 stroke-[3px]" /></div>
               </div>
               ...
               <div className="flex-1 space-y-6 text-center lg:text-left min-w-0">
                  <div className="space-y-4">
                     ...
                     <h1 className="text-2xl sm:text-4xl md:text-6xl font-black text-white tracking-tighter leading-tight antialiased uppercase">
                        {vacancy.title}
                     </h1>
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
                     <Button className="w-full sm:w-auto h-12 md:h-16 px-8 md:px-12 bg-primary hover:bg-blue-700 text-white font-black uppercase text-[10px] md:text-xs tracking-widest rounded-2xl shadow-4xl border-none transition-all active:scale-95 group/btn" asChild>
                     ...
                     <div className="flex gap-3">
                        <button onClick={handleShare} className="h-12 w-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-all active:scale-90 cursor-pointer"><Share2 className="h-5 w-5" /></button>
                        <button className="h-12 w-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-all active:scale-90 cursor-pointer"><Bookmark className="h-5 w-5" /></button>
                     </div>
                  </div>
               </div>
            </div>
         ...
         <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 px-1">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-14">
            <div className="lg:col-span-8 space-y-10 md:space-y-16">
               ...
               <HubContainer label="Recruitment details" icon={ClipboardList}>
                  ...
                  {vacancy.locationDetail && (
                     <div className="pt-8 mt-8 border-t border-slate-50 space-y-3">
                        ...
                        <p className="text-sm md:text-lg font-bold text-slate-600 leading-relaxed antialiased">{vacancy.locationDetail}</p>
                     </div>
                  )}
               </HubContainer>
               ...
               <HubContainer label="Eligibility criteria" icon={ShieldCheck} color="text-blue-500">
                  <div className="space-y-8">
                     <div className="space-y-2">
                        ...
                        <p className="text-sm md:text-xl font-[700] text-[#0F172A] leading-relaxed antialiased">{vacancy.qualificationDetail || vacancy.education}</p>
                     </div>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
               ...
               <HubContainer label="Selection process" icon={Target} color="text-rose-500">
                  <div className="space-y-8">
                     <p className="text-sm md:text-xl font-[700] text-[#0F172A] leading-relaxed antialiased">{vacancy.selectionProcess}</p>
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {["Written Exam", "Physical Test", "Medical Audit", "Final Merit"].map((stage, i) => (
                           <div key={i} className="flex items-center gap-4 p-4 md:p-5 bg-slate-50 rounded-xl border border-slate-100 group hover:bg-white hover:shadow-lg transition-all duration-500">
                              ...
                              <span className="text-xs md:text-base font-[800] text-slate-700 uppercase tracking-tight">{stage}</span>
                           </div>
                        ))}
                     </div>
                  </div>
            ...
            <aside className="lg:col-span-4 space-y-6 md:space-y-10">
               <Card className="border-none shadow-xl rounded-2xl md:rounded-[2.5rem] bg-[#0F172A] text-white p-6 md:p-10 space-y-10 relative overflow-hidden group border border-white/5">
                  ...
                  <div className="relative z-10 space-y-8 text-left">
                     <div className="space-y-1">
                        <h3 className="text-2xl md:text-3xl font-black tracking-tight leading-none uppercase">Registry</h3>
                     ...
                     <div className="pt-8 border-t border-white/5">
                        <Button asChild className="w-full h-12 md:h-14 bg-primary hover:bg-blue-700 text-white font-black uppercase text-[10px] tracking-widest rounded-xl shadow-xl border-none transition-all active:scale-95 group/portal">
               ...
               <div className="p-6 md:p-10 bg-white rounded-2xl md:rounded-[2.5rem] border border-slate-100 shadow-xl space-y-8 text-left group hover:translate-y-[-4px] transition-all duration-500">
         ...
         <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[1000] w-[95vw] max-w-3xl animate-in slide-in-from-bottom-12 duration-500">
            <div className="bg-[#0F172A]/95 backdrop-blur-xl p-3 md:p-4 rounded-2xl md:rounded-[2.5rem] shadow-5xl border border-white/10 flex items-center justify-between gap-3 md:gap-6">
               ...
               <div className="flex-1 flex items-center gap-2">
                  <button onClick={handleShare} className="h-10 md:h-14 w-10 md:w-14 rounded-xl bg-white/5 text-white hover:bg-white/10 border-none shadow-inner transition-all flex items-center justify-center cursor-pointer">
                    <Share2 className="h-4 w-4 md:h-5 md:w-5" />
                  </button>
                  <Button asChild className="flex-1 h-11 md:h-16 bg-primary hover:bg-blue-700 text-white font-black uppercase text-[10px] tracking-widest rounded-xl md:rounded-[2rem] shadow-4xl border-none active:scale-95 transition-all">
  ...
  return (
     <div className="space-y-4 md:space-y-6 group">
        <div className="flex items-center gap-4 px-2">
           <div className={cn("h-9 w-9 md:h-12 md:w-12 rounded-xl bg-slate-50 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform", color.replace('text', 'bg').replace('600', '50'))}>
              <Icon className={cn("h-5 w-5 md:h-6 md:w-6", color)} />
           </div>
           <h3 className="text-lg md:text-3xl font-black text-[#0F172A] tracking-tighter uppercase">{label}</h3>
        </div>
        <div className="bg-white border border-slate-50 shadow-xl rounded-2xl md:rounded-[3rem] p-6 md:p-14 text-left transition-all duration-500 hover:shadow-2xl">
           {children}
        </div>
     </div>
  )
...
function DataPoint({ label, value, highlight = false }: any) {
  return (
     <div className="space-y-1.5">
        <p className="text-[9px] md:text-[11px] font-black uppercase text-slate-300 tracking-widest">{label}</p>
        <p className={cn("text-sm md:text-2xl font-[800] leading-tight antialiased truncate", highlight ? "text-primary" : "text-slate-700")}>{value || "TBD"}</p>
     </div>
  )
}
...
function StatPill({ icon: Icon, label, val, color, bg }: any) {
  return (
     <Card className="border-none shadow-lg rounded-xl md:rounded-2xl bg-white p-3 md:p-6 flex flex-col items-center justify-center text-center gap-2 group hover:translate-y-[-4px] transition-all duration-500 border border-slate-50">
        <div className={cn("h-9 w-9 md:h-12 md:w-12 rounded-xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform", bg, color)}>
           <Icon className="h-4 w-4 md:h-6 md:w-6" />
        </div>
        <div className="space-y-0.5 min-w-0 w-full">
           <p className="text-xs md:text-2xl font-black text-[#0F172A] tabular-nums tracking-tighter leading-none truncate">{val}</p>
           <p className="text-[8px] md:text-[9px] font-black uppercase text-slate-400 tracking-tight truncate">{label}</p>
        </div>
     </Card>
  )
}
...
function SidebarDate({ label, val, icon, highlight = false }: any) {
  return (
     <div className={cn("flex items-center justify-between p-4 rounded-xl md:rounded-2xl border-2 transition-all group", highlight ? "bg-white/10 border-white/20 shadow-2xl scale-[1.02]" : "bg-white/5 border-transparent hover:border-white/10 hover:bg-white/10")}>
        <div className="flex items-center gap-3">
           <div className="h-8 w-8 bg-white/10 rounded-lg flex items-center justify-center shadow-inner group-hover:rotate-12 transition-transform">{icon}</div>
           <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">{label}</span>
        </div>
        <span className="text-xs md:text-lg font-black tabular-nums tracking-tighter text-white">{val}</span>
     </div>
  )
}
...
function AssetLink({ label, href, icon: Icon, color }: any) {
   if (!href) return null;
   return (
      <a href={href} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-4 bg-slate-50/50 hover:bg-white rounded-2xl border border-slate-100 transition-all group hover:shadow-xl active:scale-95">
         <div className="flex items-center gap-4">
            <div className={cn("h-9 w-9 rounded-lg flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform", color)}>
               <Icon className="h-4 w-4" />
            </div>
            <span className="text-xs md:text-base font-bold text-slate-700 group-hover:text-[#0F172A] transition-colors">{label}</span>
         </div>
