...
/**
 * @fileOverview Official Test Rules Hub v6.2 [Terminology Updated].
 */
      ...
      try {
        ...
        if (user && profile) {
          ...
          if (isAdmin) {
            ...
            if (!isPassActive) {
              setAccessError("Your Elite Pass is required for this premium test.");
            } else {
              setAccessChecked(true);
            }
          }
        ...
        setAccessError("Database sync failure.");
      } finally {
        setIsLoading(false);
      }
    }
     ...
     <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        <Card className="max-w-md w-full bg-white rounded-[3rem] p-10 md:p-14 shadow-5xl border-none space-y-8 animate-in fade-in zoom-in-95">
           ...
           <div className="space-y-3">
              <h2 className="text-2xl md:text-3xl font-black text-[#0F172A]">Test Not Found</h2>
              <p className="text-slate-500 font-medium text-sm md:text-base leading-relaxed">This test is unavailable or has been archived.</p>
           </div>
      ...
      <main className="flex-1 container mx-auto px-4 md:px-6 py-8 md:py-16 max-w-5xl text-left pb-safe">
        <div className="space-y-8 md:space-y-12">
           <div className="flex flex-col items-start gap-4">
              ...
              <h1 className="text-2xl md:text-5xl lg:text-6xl font-black text-[#0F172A] tracking-tight leading-tight">{mock?.title}</h1>
           </div>

           <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              <StatPlate icon={<Clock />} label="Time" val={`${mock?.duration || 0}m`} />
           ...
           <Card className="border-none shadow-3xl rounded-[2.5rem] md:rounded-[3.5rem] bg-white overflow-hidden border border-slate-100">
              <CardHeader className="p-8 md:p-12 bg-slate-50/50 border-b border-slate-100">
                 <CardTitle className="text-lg md:text-3xl font-black text-[#0F172A] flex items-center gap-4">
                   ...
                   {isFinished ? (
                      <div className="w-full space-y-4">
                        <Button 
                          ...
                          className="w-full h-16 md:h-20 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl shadow-xl transition-all active:scale-95 border-none flex items-center justify-center gap-3"
                        >
                        ...
                        <Button 
                          ...
                          variant="outline"
                          className="w-full h-14 border-2 border-slate-100 text-[#0F172A] font-bold rounded-2xl shadow-sm hover:bg-slate-50 active:scale-95 gap-3"
                        >
                      ...
                      <Button 
                        ...
                        className="relative overflow-hidden w-full h-16 md:h-24 bg-gradient-to-r from-blue-600 to-cyan-500 hover:brightness-110 text-white font-black rounded-[20px] md:rounded-[3rem] shadow-4xl transition-all active:scale-95 border-none group"
                      >
                         <div className="flex items-center justify-center gap-4 relative z-10">
                            <Play className="h-6 w-6 md:h-8 md:w-8 fill-white text-white" />
                            <span className="text-sm md:text-xl font-black uppercase tracking-widest">Start Test</span>
...
function StatPlate({ icon, label, val }: any) {
  return (
    <div className="p-5 md:p-10 bg-white rounded-2xl md:rounded-[3rem] border border-slate-100 shadow-xl text-center space-y-2 md:space-y-5 group hover:border-primary/30 transition-all">
       ...
       <p className="text-[8px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none">{label}</p>
       <p className="text-xl md:text-4xl font-black text-[#0F172A] tabular-nums leading-none">{val}</p>
    </div>
  )
}
