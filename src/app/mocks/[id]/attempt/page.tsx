...
export default function MockAttemptPage() {
  ...
  useEffect(() => {
    async function fetchQuestions() {
      ...
      try {
        const qData: any[] = []
        // In a real high-fidelity app, we fetch the documents in order of mockConfig.questionIds
        for (const id of mockConfig.questionIds) {
          const qSnap = await getDocs(query(collection(db, "questions"), where("id", "==", id)))
          if (!qSnap.empty) {
            qData.push(qSnap.docs[0].data())
          }
        }
        setQuestions(qData)
        if (remainingTime === 0) {
  ...
  // 3. Auto-Save Logic (Every 10 seconds)
  useEffect(() => {
  ...
  const submitMock = useCallback(() => {
    ...
    const resultData = {
      ...
      weakTopics: [], // Topic analysis logic would go here
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-white text-slate-900">
      <header className="h-16 border-b flex items-center justify-between px-6 bg-[#0B1528] text-white shrink-0 z-50 shadow-2xl">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-headline font-black text-sm sm:text-lg tracking-tight truncate max-w-[200px] sm:max-w-md uppercase">
            {mockConfig?.title}
          </h1>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button className="bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase text-xs px-8 h-10 rounded-xl shadow-xl transition-all">Submit Test</Button>
            </AlertDialogTrigger>
            <AlertDialogContent className="rounded-[2.5rem] border-none shadow-2xl bg-white">
              <AlertDialogHeader>
                <AlertDialogTitle className="font-headline text-2xl font-black text-slate-900">Final Submission Audit</AlertDialogTitle>
                <AlertDialogDescription className="text-slate-500 font-medium py-4 space-y-2">
                   <div className="flex justify-between border-b pb-2"><span>Total Questions:</span> <span className="font-black text-slate-900">{questions.length}</span></div>
                   <div className="flex justify-between border-b pb-2"><span>Answered:</span> <span className="font-black text-emerald-600">{Object.keys(answers).length}</span></div>
                   <div className="flex justify-between border-b pb-2"><span>Marked for Review:</span> <span className="font-black text-amber-500">{flagged.length}</span></div>
                   <div className="flex justify-between"><span>Not Visited:</span> <span className="font-black text-rose-500">{questions.length - Object.keys(answers).length}</span></div>
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter className="pt-6">
                <AlertDialogCancel className="rounded-xl font-bold">Back to Test</AlertDialogCancel>
                <AlertDialogAction onClick={submitMock} className="bg-primary hover:bg-primary/90 text-white font-bold rounded-xl px-10">
                  {isSubmitting ? "Generating Analytics..." : "Confirm & Submit"}
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
      ...
      <main className="flex flex-1 overflow-hidden">
        <div className="flex-1 overflow-y-auto p-4 sm:p-12 bg-slate-50/30 custom-scrollbar">
          <div className="max-w-4xl mx-auto space-y-10 pb-20">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black text-primary bg-primary/5 px-4 py-2 rounded-xl border border-primary/10 tracking-widest uppercase">
            ...
            <div className="space-y-10">
              <h2 className="text-2xl sm:text-4xl font-bold leading-tight text-[#0F172A] tracking-tight">
              ...
              >
                {['A', 'B', 'C', 'D'].map((key, i) => {
                  ...
                  const optionText = language === 'english' 
                    ? (question as any)[`option${key}En`] 
                    : ((question as any)[`option${key}Pa`] || (question as any)[`option${key}En`]);

                  return (
                    <div 
                      key={i} 
                      onClick={() => setAnswers(prev => ({ ...prev, [currentIdx]: i }))}
                      className={`flex items-center space-x-6 p-7 border-2 rounded-[1.5rem] transition-all duration-300 cursor-pointer group ${isSelected ? 'border-primary bg-primary/5 ring-4 ring-primary/5 shadow-lg' : 'border-white bg-white hover:border-slate-200 shadow-sm hover:shadow-md'}`}
                    >
                      <RadioGroupItem value={i.toString()} id={`opt-${i}`} className="text-primary border-slate-300 scale-125" />
                      <Label htmlFor={`opt-${i}`} className="flex-1 cursor-pointer text-xl font-bold select-none text-slate-700">
                        <span className={`mr-6 text-xs font-black uppercase tracking-[0.2em] px-3 py-1 rounded-lg ${isSelected ? 'bg-primary text-white' : 'bg-slate-100 text-slate-400'}`}>
                          {key}
                        </span>
                        {optionText}
                      </Label>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-12 border-t border-slate-200">
              <div className="flex gap-4 w-full sm:w-auto">
                <Button variant="outline" size="lg" className="flex-1 sm:flex-none font-bold rounded-2xl border-slate-200 h-14 px-8" onClick={handlePrev} disabled={currentIdx === 0}>
                  <ChevronLeft className="mr-2 h-4 w-4" /> Previous
                </Button>
                <Button className="flex-1 sm:flex-none bg-[#0F172A] hover:bg-[#1E293B] text-white font-black uppercase tracking-widest text-xs px-12 h-14 rounded-2xl shadow-2xl" onClick={handleNext} disabled={currentIdx === questions.length - 1}>
              ...
              <div className="flex gap-4 w-full sm:w-auto">
                <Button 
                  variant="ghost" 
                  className="text-slate-400 hover:text-rose-500 font-bold h-14 rounded-2xl px-6"
                ...
                <Button 
                  variant="ghost" 
                  className={`flex-1 sm:flex-none font-black uppercase tracking-widest text-[10px] transition-all h-14 px-8 rounded-2xl border-2 ${flagged.includes(currentIdx) ? "text-amber-600 bg-amber-50 border-amber-200" : "border-transparent text-slate-400"}`} 
                  onClick={toggleFlag}
                >
        ...
        <aside className="w-96 border-l border-slate-200 bg-white overflow-y-auto hidden lg:block p-10 shadow-inner">
           <QuestionPalette 
            totalQuestions={questions.length}
