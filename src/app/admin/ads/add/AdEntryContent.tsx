...
export default function AdEntryContent({ serverExistingAd, serverExams, serverAdId }: any) {
  ...
  useEffect(() => {
    if (serverExistingAd) {
      setFormData({
        ...serverExistingAd,
        targeting: serverExistingAd.targeting || { examIds: [] }
      })
    }
  }, [serverExistingAd])
  ...
  const handleSave = async () => {
    ...
    const payload: Partial<Ad> = {
      ...
      createdAt: isEditing ? (serverExistingAd?.createdAt || serverTimestamp()) : serverTimestamp(),
      stats: isEditing ? (serverExistingAd?.stats || { impressions: 0, clicks: 0 }) : { impressions: 0, clicks: 0 }
    }
