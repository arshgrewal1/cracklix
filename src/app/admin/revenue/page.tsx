...
/**
 * @fileOverview Official Revenue Governance Dashboard v1.2.
 * FIXED: Standardized header using AdminPageHeader to resolve spatial tightness.
 * UPDATED: Refined MetricCard layout and Title Case normalization.
 */
  ...
  return (
    <div className="space-y-10 md:space-y-16 text-[#0F172A] text-left animate-in fade-in duration-700 pb-32 pt-2">
      ...
      <AdminPageHeader
        ...
        subtitle="Strategic financial registry and transaction monitoring."
      />

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 px-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 px-1">
         <div className="lg:col-span-8">
            <Card className="border-none shadow-2xl rounded-[3rem] bg-white overflow-hidden border border-slate-100">
         ...
         <div className="lg:col-span-4">
            <Card className="border-none shadow-2xl rounded-[3rem] bg-[#0F172A] text-white p-8 md:p-12 space-y-12 relative overflow-hidden h-full">
               <div className="absolute top-0 right-0 p-12 opacity-5 rotate-12"><BarChart3 className="h-64 w-64 text-primary" /></div>
...
function MetricCard({ label, value, trend, icon, color, bgColor, className }: any) {
   return (
      <Card className={cn("border-none shadow-xl rounded-2xl md:rounded-[2.5rem] bg-white p-6 md:p-10 transition-all duration-500 hover:translate-y-[-4px] border border-slate-50 group h-full", className)}>
         <div className="flex items-center justify-between mb-6 md:mb-8">
            <div className={cn("h-11 w-11 md:h-14 md:w-14 rounded-xl md:rounded-2xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform", bgColor, color)}>
               {React.isValidElement(icon) ? React.cloneElement(icon as React.ReactElement, { className: "h-5 w-5 md:h-7 md:w-7" }) : icon}
            </div>
            <Badge className="bg-slate-50 text-slate-400 border-none text-[8px] font-black uppercase tracking-widest px-2 py-0.5">{trend}</Badge>
         </div>
