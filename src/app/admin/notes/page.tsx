...
import { Plus, Trash2, Edit, Save, FileText, Search, Download, ExternalLink, Globe, Landmark, Layers, GraduationCap, X } from "lucide-react"
...
/**
 * @fileOverview Institutional Download Hub Admin CMS v6.0.
 * UPDATED: Added Mandatory Exam Hub selector to link notes to verticals.
 */
  ...
  return (
    <div className="space-y-10 pb-20 text-left">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 px-4">
        ...
        <Button onClick={() => setEditingNote({ title: "", subjectId: "", examId: "", category: "NOTES", pdfUrl: "", isFree: true, description: "" })} className="bg-primary hover:bg-orange-600 gap-3 h-16 px-10 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-2xl transition-all active:scale-95 border-none">
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-slate-50 h-20">
                ...
                <TableHead className="text-[10px] font-black uppercase text-slate-500">Exam Vertical</TableHead>
            ...
            <TableBody>
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <TableRow key={i}><TableCell colSpan={5} className="px-10 py-8"><Skeleton className="h-14 w-full rounded-2xl bg-slate-50" /></TableCell></TableRow>
                ))
                ...
                return (
                  <TableRow key={note.id} className="hover:bg-slate-50 group border-slate-50 transition-all">
                    ...
                    <TableCell>
                       <p className="text-xs font-black text-[#0F172A] uppercase">{exam?.name || 'GENERIC HUB'}</p>
                    </TableCell>
                    <TableCell>
      ...
      <Dialog open={!!editingNote} onOpenChange={(open) => !open && setEditingNote(null)}>
        <DialogContent className="sm:max-w-2xl rounded-[3rem] bg-white border-none shadow-5xl p-0 overflow-hidden text-left flex flex-col">
          ...
          <DialogHeader className="p-10 pb-0 flex flex-row items-center justify-between">
            ...
            <button onClick={() => setEditingNote(null)} className="p-2 rounded-xl hover:bg-slate-50 transition-colors"><X className="h-5 w-5 text-slate-400" /></button>
          </DialogHeader>
          ...
          <div className="p-10 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase text-slate-500 ml-1 flex items-center gap-2"><GraduationCap className="h-3 w-3" /> Target Exam Hub (Mandatory)</Label>
              <select 
                value={editingNote?.examId || ""} 
                onChange={e => setEditingNote({...editingNote, examId: e.target.value})} 
                className="w-full h-14 bg-slate-50 border-none rounded-xl px-4 font-black text-sm outline-none shadow-inner"
              >
            ...
            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase text-slate-500 ml-1 flex items-center gap-2"><Globe className="h-3 w-3" /> Institutional PDF URL</Label>
              <Input value={editingNote?.pdfUrl || ""} onChange={e => setEditingNote({...editingNote, pdfUrl: e.target.value.trim()})} className="h-14 rounded-xl border-slate-100 bg-slate-50 font-bold text-primary" placeholder="https://cloud.storage/note.pdf" />
            </div>
          ...
          <DialogFooter className="p-10 pt-4 bg-slate-50 flex gap-4 border-t border-slate-100">
            <Button variant="ghost" onClick={() => setEditingNote(null)} className="rounded-xl h-14 font-black uppercase text-[10px] text-slate-400">Cancel Draft</Button>
            <Button onClick={handleSave} className="bg-[#0F172A] hover:bg-black text-white h-14 px-10 rounded-xl font-black uppercase text-[10px] tracking-widest flex-1 shadow-xl transition-all active:scale-95 border-none">
