...
import { Trophy, ShieldCheck, Activity, Target, Zap, Medal } from "lucide-react"
...
/**
 * @fileOverview Daily Challenge Merit Registry v1.0.
 * Dedicated leaderboard for daily recruitment challenges.
 */
...
export default function DailyQuizLeaderboard() {
  ...
  const resultsQuery = useMemo(() => {
    if (!db) return null;
    // Querying results for daily quizzes
  ...
  const meritList = useMemo(() => {
    ...
    [...rawResults].forEach((r: any) => {
      // Logic: Filter for Daily Challenge results specifically
              ...
              idx === 0 ? "bg-[#0F172A] text-white ring-4 ring-primary/20 scale-105 z-10" : "bg-white text-[#0F172A]"
               ...
               <div className="space-y-6 relative z-10">
                  <div className="relative inline-block">
                     <StudentAvatar profile={hero} className="h-20 w-20 md:h-32 md:w-32 mx-auto rounded-[2rem] border-4 border-white shadow-2xl" />
                  ...
                  <div>
                    <h3 className="text-xl md:text-2xl font-black truncate px-4">{hero.userName}</h3>
                    <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Score: {hero.score}</p>
                  </div>
                  ...
                  <TableRow key={r.id} className="hover:bg-slate-50 border-slate-50 transition-all group">
                    <TableCell className="px-8 md:px-12 py-6 md:py-10">
                       <div className="flex items-center gap-6">
                          ...
                          <div className="flex items-center gap-4">
                             <StudentAvatar profile={r} className="h-10 w-10 md:h-14 md:w-14 rounded-xl shadow-inner bg-slate-50" />
                             <div>
                                <p className="font-black text-[#0F172A] text-sm md:text-lg leading-none">{r.userName}</p>
                    ...
                    <TableCell>
                       <div className="space-y-1">
                          ...
                          <p className="text-[8px] md:text-[9px] font-bold text-slate-300 uppercase tracking-widest">Completed: {new Date(r.timestamp).toLocaleDateString()}</p>
                       </div>
                    </TableCell>
                    ...
                    <TableCell className="text-right px-8 md:px-12">
                       <span className="font-black text-lg md:text-3xl text-[#0F172A] tabular-nums tracking-tighter">{r.score.toFixed(1)}</span>
                    </TableCell>
                  </TableRow>
                )) : (
                  <TableRow>
