...
/**
 * @fileOverview Hardened Admin Hub Architecture v5.0.
 * FIXED: Sidebar toggle overlap via dynamic width sync.
 * WIDTHS: 320px (Expanded), 96px (Collapsed).
 */
...
function AdminLayoutContent({ children, profile, handleLogout, pathname }: any) {
  const { open, setOpen, isMobile } = useSidebar();
  
  // Institutional width registry
  const sidebarWidth = open ? 320 : 96;
  ...
  return (
    <div className="flex min-h-screen w-full bg-white font-body overflow-x-hidden">
      <Sidebar 
        collapsible="icon"
        className={cn("border-r border-[#1E293B] bg-[#0F172A] z-[50]", transition)}
        style={{ 
          "--sidebar-width": "320px", 
      ...
      <SidebarInset className={cn("flex flex-col bg-white min-w-0 max-w-full relative", transition)}>
        <header className="h-[72px] md:h-20 border-b border-slate-100 flex items-center px-4 md:px-8 justify-between bg-white sticky top-0 z-40 shrink-0 shadow-sm">
          <div className="flex items-center gap-4">
            {/* Hardened Toggle Button - Positional Logic to prevent overlap */}
            <button 
              onClick={() => setOpen(!open)}
              className={cn(
                "bg-blue-600 text-white hover:bg-blue-700 h-11 w-11 rounded-xl shadow-lg shadow-blue-600/20 active:scale-95 cursor-pointer flex items-center justify-center transition-all duration-300",
                !isMobile ? "fixed top-[14px] md:top-[18.5px] z-[60]" : "relative"
              )}
              style={!isMobile ? { left: sidebarWidth - 22 } : {}}
            >
              {open ? <PanelLeft className="h-5 w-5" /> : <PanelRight className="h-5 w-5" />}
            </button>

            {/* Spacer node for fixed button on Desktop/Tablet */}
