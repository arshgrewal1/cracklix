                ...
                <div className="grid grid-cols-1 gap-8">
                   {sections.map((sec: any, sIdx: number) => (
                      <Card key={sec.id} className="border-none shadow-3xl rounded-[3rem] bg-white overflow-hidden border border-slate-100 group/sec">
                         <div className="flex items-center justify-between p-8 bg-slate-50/50 border-b border-slate-50"><div className="flex items-center gap-6"><div className={cn("h-12 w-12 rounded-2xl flex items-center justify-center font-black text-xl shadow-xl transition-colors", activeSectionId === sec.id ? "bg-primary text-white" : "bg-[#0F172A] text-white")}>{sIdx + 1}</div><div className="text-left"><Input value={sec.name} onChange={e => setSections((p: any[]) => p.map((s: any) => s.id === sec.id ? { ...s, name: e.target.value.toUpperCase() } : s))} className="h-8 p-0 bg-transparent border-none font-black uppercase text-xl md:text-2xl focus-visible:ring-0 text-[#0F172A]" /><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{(sec.questions?.length || 0)} Linked Preparation Nodes</p></div></div><div className="flex gap-2"><Button onClick={() => setActiveSectionId(sec.id)} variant="ghost" className={cn("h-10 px-6 rounded-xl font-black uppercase text-[9px] tracking-widest", activeSectionId === sec.id ? "bg-primary text-white" : "text-slate-400")}>{activeSectionId === sec.id ? "ACTIVE" : "FOCUS"}</Button><Button onClick={() => setSections((p: any[]) => p.filter((s: any) => s.id !== sec.id))} className="h-10 w-10 text-rose-500 hover:bg-rose-50 rounded-xl transition-all flex items-center justify-center border-none shadow-none bg-transparent" variant="ghost"><Trash2 className="h-5 w-5" /></Button></div></div>
