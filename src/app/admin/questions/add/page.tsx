...
/**
 * @fileOverview Manual MCQ Ingestion Node v4.0.
 * RESTORED: Side-by-side bilingual Typing HUB.
 */
...
function QuestionEntryContent() {
  ...
  const handleSave = async () => {
    ...
    if (!formData.englishQuestion && !formData.punjabiQuestion) {
       toast({ variant: "destructive", title: "Validation Error", description: "Subject and Statement are mandatory." })
       return
    }
    ...
    const payload: any = { 
      ...
      isStandalone: true,
      author: existingData?.author || profile?.name || "Manual Node",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
        <div className="lg:col-span-7 space-y-8">
           <Card className="border-none bg-white shadow-2xl rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-12 space-y-10 border border-slate-50">
                    ...
                    {['A','B','C','D'].map(opt => (
                       <div key={opt} className="bg-slate-50/50 p-6 md:p-8 rounded-[2.5rem] border border-slate-100 space-y-6 group hover:border-primary/20 transition-all shadow-sm">
                          <div className="flex items-center gap-4">
                             ...
                             <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Option {opt} Ingestion</Label>
                          </div>
              ...
              <div className="space-y-8 pt-6 border-t border-slate-50">
                 <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em] flex items-center gap-2"><BarChart3 className="h-4 w-4" /> 3. Logic Hub</p>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3 text-left">
                       <Label className="text-[10px] font-black uppercase text-slate-400 ml-1">Answer Key</Label>
                    ...
                    <div className="space-y-3 text-left">
                       ...
                       <Select value={formData.subjectId} onValueChange={v => setFormData({...formData, subjectId: v})}>
                          ...
                          <SelectContent className="bg-[#0B1528] text-white">{subjects?.map((s: any) => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent>
                       </Select>
                    </div>
                 </div>
                 <div className="grid grid-cols-1 gap-6">
                    {showEnglish && (
                       <div className="space-y-2 text-left">
                          ...
                          <Textarea value={formData.englishExplanation} onChange={e => setFormData({...formData, englishExplanation: e.target.value})} className="min-h-[140px] rounded-2xl bg-slate-900 border-none text-emerald-400 font-medium p-6 shadow-2xl leading-relaxed" placeholder="Type logic..." />
                       </div>
                    )}
                    {showPunjabi && (
                       <div className="space-y-2 text-left">
                          <Label className="text-[10px] font-black uppercase text-primary ml-1">Punjabi Rationale</Label>
        ...
        <div className="lg:col-span-5">
           <Card className="border-none shadow-5xl rounded-[3rem] overflow-hidden sticky top-32 border border-slate-100 bg-white">
             <div className="bg-[#0B1528] px-10 py-6 flex items-center justify-between">
                <div className="flex items-center gap-3"><div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" /><span className="text-[10px] font-black uppercase tracking-widest text-white">Simulation Preview</span></div>
             </div>
             <CardContent className="p-10 space-y-10 h-[75vh] overflow-y-auto custom-scrollbar text-left bg-slate-50/20">
                <QuestionRenderer language={previewLang} question={{...formData, displayId: "1"}} showSolution={true} />
             </CardContent>
          </Card>
        </div>
      </div>
